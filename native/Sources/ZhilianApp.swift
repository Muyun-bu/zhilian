    ...
    var body: some View {
        NavigationSplitView {
            VStack(spacing: 14) {
                ...
                Spacer()
                VStack(alignment: .leading, spacing: 8) { Label(model.running ? "本地代理运行中" : "本地代理已停止", systemImage: model.running ? "checkmark.circle.fill" : "pause.circle").foregroundStyle(model.running ? .green : .secondary); Text("127.0.0.1:\(model.config.proxyPort)").font(.caption.monospaced()).foregroundStyle(.secondary) }.padding()
